...
export function GoogleSignInButton({ onSuccess }: GoogleSignInButtonProps) {
  ...
  return (
    <div className="w-full">
      {/* Hidden Google button — handles the actual GSI flow */}
      <div ref={hiddenRef} className="absolute -left-[9999px]" aria-hidden="true" />

      {/* Custom visible button */}
      <button
        type="button"
        onClick={handleClick}
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] px-4 py-3 text-sm font-medium text-[var(--foreground)] transition-all duration-200 hover:bg-[var(--surface-1)] hover:border-[var(--glass-border)] hover:shadow-sm active:scale-[0.98]"
      >
